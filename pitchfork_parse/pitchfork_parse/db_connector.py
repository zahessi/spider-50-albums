...
class DBconnector:
    ...
    def _open_close_connection(self):
        connection = MySQLdb.connect(host=self.db_config["host"],  # your host, usually localhost
                                     user=self.db_config["user"],  # your username
                                     passwd=self.db_config["password"],  # your password
                                     db=self.db_config["database"])  # name of the data base
